...
RATE_LIMIT = 8  # seconds between arXiv requests
...
def fetch_papers() -> list[dict]:
    ...
    for cat in CATEGORIES:
        try:
            papers = search_by_category(cat, MAX_PER_CATEGORY)
        except Exception as e:
            print(f"  WARNING: Failed to fetch {cat}: {e}, skipping this category", file=sys.stderr)
            continue
        for p in papers:
...
def generate_digest(papers: list[dict]) -> str:
    ...
    html_digest = call_deepseek(messages, temperature=0.3, max_tokens=8192)
    return html_digest
